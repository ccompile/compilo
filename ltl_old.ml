...
let write2 c r l = match lookup c r with
  | Reg hr -> hr, l
  | Spilled n -> tmp2, generate (Eset_stack (tmp2, n, l))
...
let read2 c r f = match lookup c r with
  | Reg hr -> f hr
  | Spilled n -> Eget_stack (tmp2, n, generate (f tmp1))
...
let instr c frame_size = function
  ...
  | Ertl.Embinop (r1, op, r2, r3, l) ->
    ...
    let hw1, l = write1 c r1 l in
    Lmbinop (hw1, op, hw2, hw3, l))
...
let deffun t =
  ...
  in
  let frame size = 4 * (nlocals + n_stack_params) in
  graph := Label.M.empty;
